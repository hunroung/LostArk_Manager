...
// LostArk character class
  ...
  class Char_info {
    public:
        ...
        static const int MAX_identify_LENGTH = 60; //클래스 이름 최대 길이
        static const int MAX_NAME_LENGTH = 60;     //캐릭터 이름 최대 길이
        ...
        int p_total_count() { return total_char_count; }
        char* p_main_name() { return main_char; }
        char* p_all_name(int i) { return name[i]; }
        ...
        char* p_gackin_name(int i) { return gackin_name[i]; }
        int p_error() { return errorcode; }
    private:
